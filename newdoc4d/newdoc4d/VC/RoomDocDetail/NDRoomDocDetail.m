...
- (void)setupUI{
    ...
    [self startGetDoctorIntroWithDocId:self.doctor.ID success:^(NDDoctorIntro *intro) {
        ...
        [weakself.btnHeader sd_setImageWithURL:[NSURL URLWithString:intro.picture_url] forState:UIControlStateNormal];
        weakself.lblDocName.text = SafeString(weakself.doctor.name);
        weakself.lblDocTitle.text = SafeString(weakself.doctor.title);
        weakself.lblGoodat.text = SafeString([NSString stringWithFormat:@"擅长：%@", weakself.doctor.goodat]);
        NDSubroom *subroom = weakself.doctor.catalog[0];
        weakself.lblSubroom.text = SafeString([NSString stringWithFormat:@"科室：%@",subroom.name]);
        weakself.lblEducation.text = SafeString([NSString stringWithFormat:@"学历：%@",intro.graduate_college]);
